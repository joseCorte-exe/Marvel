...
Home.getInitialProps = async function ({req, query: {id}}) {
    ...
    await api.get('series?ts=1643487890&apikey=ff71f49761fb6a07a7b94a1fe4a112d3&hash=fcf44c5101f8b675183d3f9a00b437c3')
                ...
                <section>
                    {
                        catalog.map(value => <Card key={value.id} id={value.id} image={value.thumbnail.path}
                            ...
                            creators={value.creators.items.map(value => (value.name))} route={`/series/${value.id}`}/> )
